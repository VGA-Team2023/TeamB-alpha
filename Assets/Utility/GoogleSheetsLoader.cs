...
namespace TeamB_TD.Utility
{
    public static class GoogleSheetsLoader
    {
        ...
        public static async UniTask<List<string[]>> LoadGoogleSheetsAsync(string sheetID, string sheetName, int ignoreRows = 0, CancellationToken token = default)
        {
            var url = "https://docs.google.com/spreadsheets/d/" + sheetID + "/gviz/tq?tqx=out:csv&sheet=" + sheetName;
            List<string[]> data = new List<string[]>();
            ...
            using (UnityWebRequest request = UnityWebRequest.Get(url))
            {
                ...
                try
                {
                    await operation.ToUniTask(cancellationToken: token); // UniTaskに変換して非同期処理を待機
                }
